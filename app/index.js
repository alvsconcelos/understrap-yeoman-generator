...
var understrapThemeGenerator = module.exports = function understrapThemeGenerator(args, options, config) {
...
util.inherits(understrapThemeGenerator, yeoman.generators.Base);

understrapThemeGenerator.prototype.askFor = function askFor() {
  var cb = this.async();
...
understrapThemeGenerator.prototype.installunderscores = function installunderscores() {
  this.startertheme = 'https://github.com/alvsconcelos/understrap_native/archive/master.tar.gz';
...
function findandreplace(dir) {
  ...
  files.forEach(function (file) {
    ...
    if (stat.isFile() && (path.extname(file) == '.php' || path.extname(file) == '.css')) {
      ...
      var theme_slug = slugify(self.themename, { replacement: '_', lower: true });
      var result;

      // themetextdomain - text domain
      // themenamepackage - theme package name
      // themenamefunction_ - theme functions
      // themenametag- - style tags etc      

      result = data.replace(/themetextdomain/g, theme_slug);
      result = result.replace(/themenamepackage/g, theme_slug);
      result = result.replace(/themenamefunction_/g, theme_slug + "_");
      result = result.replace(/themenametag-/g, theme_slug + "-");
      ...
      if (file == 'style.css') {
        ...
        self.log.info('Updating theme information in ' + file);
        result = result.replace(/authorurl/g, self.authoruri);
        result = result.replace(/authorname/g, self.author);
      }
...
understrapThemeGenerator.prototype.renameunderscores = function renameunderscores() {
  findandreplace.call(this, '.');
  this.log.ok('Done replacing string ' + chalk.yellow('_s'));
};

// understrapThemeGenerator.prototype.addfiles = function addfiles() {
...
// understrapThemeGenerator.prototype.sassboostrap = function sassboostrap() {
